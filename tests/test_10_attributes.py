...
def test_GetAttributeIdByName():
    assert isinstance(rt.rtapi.GetAttributeIdByName('HW type'), long) == True

def test_GetAttributeId():
    assert isinstance(rt.rtapi.GetAttributeId('HW type'), long) == True

def test_GetDictionaryChapterId():
    chapter_id = rt.rtapi.GetDictionaryChapterId('Yes/No')
    assert isinstance(chapter_id, long) == True
...
def test_GetDictionaryIdByValue():
    ...
    assert isinstance(test_dict_value, long) == True 
